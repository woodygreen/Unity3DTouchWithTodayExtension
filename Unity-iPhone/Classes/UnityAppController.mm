...
{    
    ...
    return [[DemoInner sharedDemo] CallUnity:gameObject i_methodName:methodName i_param:param i_unity:_unityAppReady];
}

-(void) RequestActiveTmer
{
    [[DemoInner sharedDemo] CallUnity:nil i_methodName:@"RequestActiveTimer" i_param:nil i_unity:_unityAppReady];
    return;
}

void SetActiveTimer(const char* i_timer)
{
    return;
}
...
long GetBestScore()
{
    NSString * score = [[DemoInner sharedDemo] LoadLocalData:@"FlappyBirdHighScore"];
    if(nil == score)
    {
        score = @"0";
    }
...
void FinishedUnityInterfaceStarting()
{
    NSString * methodName = [[DemoInner sharedDemo] LoadLocalData:@"UnityMethod"];
    NSString * param = [[DemoInner sharedDemo] LoadLocalData:@"UnityParam"];
    if(nil != param)
    {
        [[DemoInner sharedDemo] SaveLocalData:@"UnityParam" i_value:nil];
    }
    
    if(nil != methodName)
    {
        [[DemoInner sharedDemo] SaveLocalData:@"UnityMethod" i_value:nil];
        [[DemoInner sharedDemo] CallUnity:nil i_methodName:methodName i_param:param i_unity:_unityAppReady];
    }
    return;
}
...
{
    if ([url.absoluteString hasPrefix:@"sdgExtension"])
    {
        if ([url.absoluteString hasSuffix:@"randomchart"])
        {
            [[DemoInner sharedDemo] CallUnity:nil i_methodName:@"AddRandomChart" i_param:nil i_unity:_unityAppReady];
        }
        else if ([url.absoluteString hasSuffix:@"weeklysign"])
        {
            [[DemoInner sharedDemo] CallUnity:nil i_methodName:@"WeeklySign" i_param:nil i_unity:_unityAppReady];
        }
        else if ([url.absoluteString hasSuffix:@"auctionHouse"])
        {
            [[DemoInner sharedDemo] CallUnity:nil i_methodName:@"ActionHouse" i_param:nil i_unity:_unityAppReady];
        }
        else if ([url.absoluteString hasSuffix:@"refreshActive"])
        {
            [self RequestActiveTmer];
        }
    }
...
- (void)applicationDidBecomeActive:(UIApplication*)application
{
    ...
    NSString * gridChart = [[DemoInner sharedDemo] LoadLocalData:@"gridChart"];
    if(nil != gridChart)
    {
        [[DemoInner sharedDemo] CallUnity:nil i_methodName:@"AddChart" i_param:gridChart i_unity:_unityAppReady];
    }
